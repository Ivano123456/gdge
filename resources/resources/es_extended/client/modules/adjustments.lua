...
local function multipliersActive()
    local m = Config.Multipliers
    return m.pedDensity ~= 1.0
        or m.scenarioPedDensityInterior ~= 1.0
        or m.scenarioPedDensityExterior ~= 1.0
        or m.ambientVehicleRange ~= 1.0
        or m.parkedVehicleDensity ~= 1.0
        or m.randomVehicleDensity ~= 1.0
        or m.vehicleDensity ~= 1.0
end

function Adjustments:FrameLoop()
    local needsAmmo = Config.DisableDisplayAmmo
    local needsRewards = Config.DisableVehicleRewards
    local needsMultipliers = multipliersActive()

    if not needsAmmo and not needsRewards and not needsMultipliers then
        return
    end
    ...
    CreateThread(function()
        while true do
            ...
            if needsRewards then
                DisablePlayerVehicleRewards(ESX.playerId)
            end

            if needsMultipliers then
