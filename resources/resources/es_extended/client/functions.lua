...
local function NormalizeNotifyType(notifyType)
    if type(notifyType) == "boolean" then
        return notifyType and "info" or "error"
    end

    if type(notifyType) ~= "string" or notifyType == "" then
        return "info"
    end

    local normalized = string.lower(notifyType)
    if normalized == "success" then
        return "success"
    end

    if normalized == "error" or normalized == "warning" or normalized == "info" or normalized == "success" then
        return normalized
    end
...
local function IsResourceFound(resource)
	return addonResourcesState[resource] == true
end
...
function ESX.ShowNotification(message, notifyType, length, title, position)
	...
	local time = type(length) == "number" and length > 0 and length or 5000
