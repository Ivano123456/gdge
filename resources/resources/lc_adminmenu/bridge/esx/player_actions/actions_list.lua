...
local PlayerActions = {
  {
    ...
    onClick = function(player, args)
      ...
      local attempt_give_item = lib.callback.await('lc_admin:player_actions:esx:attemptGiveItem', 100, tonumber(player.source), args[1], args[2])
  ...
  },
  {
    ...
    onClick = function(player)
      TriggerServerEvent('lc_admin:player_actions:esx:heal', player.source)
    end
  },
  {
    ...
    onClick = function(player)
      CloseAdminMenu()
      TriggerServerEvent('lc_admin:player_actions:esx_ox:viewInventory', player.source)
    end
  },
  {
    ...
    onClick = function(player)
      TriggerServerEvent('lc_admin:player_actions:esx_ox:clearInventory', player.source)
    end
  },
  {
    label = L('actions.confiscate_inv'),
    permission = 'clear_inventory',
    onClick = function(player)
      TriggerServerEvent('lc_admin:player_actions:esx_ox:confiscateInventory', player.source)
    end
  },
  {
    label = L('actions.return_inv'),
    permission = 'clear_inventory',
    onClick = function(player)
      TriggerServerEvent('lc_admin:player_actions:esx_ox:returnInventory', player.source)
    end
  },
  {
    ...
    onClick = function(player, args)
      ...
      TriggerServerEvent('lc_admin:player_actions:esx:setBucket', player.source, args[1])
    end
  }
}
