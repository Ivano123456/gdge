window.APP = {
	...
	data() {
		return {
			...
			showWindow: false,
			shouldHide: true,
			chatUserDisabled: false,
			backingSuggestions: [],
			removedSuggestions: [],
			templates: {
				'default': '<div class="chat-message msg-default"><i class="far fa-comment" style="margin-right:6px;"></i><b>{0}</b><div class="msg-body">{1}</div></div>',
				'defaultAlt': '{0}',
				'print': '<pre>{0}</pre>',
	...
	mounted() {
		post('http://jamaica-chat/loaded', JSON.stringify({}));
	...
	methods: {
		ON_SCREEN_STATE_CHANGE({ shouldHide }) {
			this.shouldHide = this.chatUserDisabled ? true : shouldHide;
		},
		ON_CHAT_USER_TOGGLE({ disabled }) {
			this.chatUserDisabled = !!disabled;
			if (!this.chatUserDisabled) {
				return;
			}
		...
		send(e) {
			if(this.message !== '') {
				post('http://jamaica-chat/chatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('http://jamaica-chat/chatResult', JSON.stringify({ canceled }));
			}
			this.message = '';
			this.showInput = false;
			clearInterval(this.focusTimer);
