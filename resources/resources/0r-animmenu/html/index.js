// Preview görselleri Cloudflare R2 (kendi domain) üzerinden servis ediliyor.
// Host değiştirmek istersen sadece bu iki satırı düzenle.
const PREVIEW_STATIC_BASE = "https://cdn.0resmonanimmenu.com/static";
const PREVIEW_HOVER_BASE = "https://cdn.0resmonanimmenu.com/hover";
let menuOpen = false;
...
let animations = [];
let defaultThemeFromConfig = 'modern';
let groupState = { enabled: false, inGroup: false, isOwner: false, owner: null, maxPlayers: 5, members: [] };
let groupActiveAnim = null;
let groupAnimationRunning = false;
let groupCategoryData = null;
...
window.addEventListener('message', function (event) {
    ...
    if (ed.action === "textUI") {
        ...
        menuOpen = ed.state;
        theme = localStorage.getItem('themev2');
        if (theme === null) {
            localStorage.setItem('themev2', defaultThemeFromConfig);
            theme = defaultThemeFromConfig;
        }
        if (menuOpen) {
            if (ed.menu) {
    ...
    } else if (ed.action === "updateGroup") {
        const wasInGroup = groupState && groupState.inGroup;
        groupState = ed.group || groupState;
        if (!groupState.inGroup) groupAnimationRunning = false;
        updateGroupCategoryText();
        if (currentCategory === "group") renderGroupPage();
        if (groupModalOpen) {
            if (wasInGroup && !groupState.inGroup) {
                closeGroupModal(false);
            } else {
                renderGroupModal();
            }
        }
    } else if (ed.action === "updateGroupActiveAnim") {
        groupActiveAnim = ed.anim || null;
        if (currentCategory === "group") renderGroupPage();
        if (groupModalOpen) renderGroupModal();
    } else if (ed.action === "updateGroupAnimationRunning") {
        groupAnimationRunning = ed.running === true;
        if (currentCategory === "group") renderGroupPage();
        if (groupModalOpen) renderGroupModal();
        ...
        if (ed.quickAnimationsState !== undefined) {
            ...
            if (quickAnimationsState === true) {
                ...
                document.getElementById("mainContainerRightAnimations").style.height = "55.5vh";
                document.getElementById("mainContainerRightTop").style.marginTop = "1.6vh";
            } else {
                ...
                document.getElementById("mainContainerRightAnimations").style.height = "76vh";
                document.getElementById("mainContainerRightTop").style.marginTop = "1.6vh";
            }
        }
        if (ed.quickKeys) {
        ...
        theme = localStorage.getItem('themev2');
        if (ed.themeCategories) {
            for (const [themeName, themeData] of Object.entries(ed.themeCategories)) {
                if (themeData.isDefault) {
                    defaultThemeFromConfig = themeName.toLowerCase();
                    break;
                }
            }
            const dropdown1 = document.getElementById('interfaceDropdownMenu1');
            const dropdown2 = document.getElementById('interfaceDropdownMenu2');
            if (dropdown1) {
                dropdown1.innerHTML = '';
                ...
                for (const [themeName, themeData] of Object.entries(ed.themeCategories)) {
                    if (themeData.Enable) {
                        ...
                        a.href = '#';
                        a.setAttribute('data-interface', interfaceIndex.toString());
                        a.className = themeName.toLowerCase();
            ...
            if (dropdown2) {
                ...
                const showCatA = document.createElement('a');
                showCatA.href = '#';
                showCatA.setAttribute('data-interface', (Object.keys(ed.themeCategories).length + 1).toString());
                showCatA.className = 'show_categories';
                showCatA.id = 'show_hide_categories';
                dropdown2.appendChild(showCatA);
            }
            if (theme !== null) {
                let isCurrentThemeEnabled = false;
                for (const [themeName, themeData] of Object.entries(ed.themeCategories)) {
                    if (themeName.toLowerCase() === theme && themeData.Enable) {
                        isCurrentThemeEnabled = true;
                        break;
                    }
                }
                if (!isCurrentThemeEnabled) {
                    localStorage.setItem('themev2', defaultThemeFromConfig);
                    theme = defaultThemeFromConfig;
                }
            }
        ...
        if (ed.group) groupState = ed.group;
        if (ed.groupAnimationRunning !== undefined) groupAnimationRunning = ed.groupAnimationRunning === true;
        ...
        document.getElementById("mainContainerRightAnimations").innerHTML = "";
        document.getElementById("mainContainer2BottomRightInside").innerHTML = "";
        if (theme === "modern") {
        ...
        groupCategoryData = Array.isArray(ed.categories) ? ed.categories.find((categoryData) => categoryData.name === "group" || categoryData.isGroup) : null;
        renderGroupShortcutButton();
        currentCategory = null;
        if (theme === "modern") {
            ed.categories.forEach(function (categoryData, index) {
                if (categoryData.name === "group" || categoryData.isGroup) return;
                ...
                if (translations && translations.animations_number) {
                    anim_number_text = translations.animations_number;
                }
                const categoryInfo = categoryData.isGroup ? getGroupCategoryText() : `${categoryData.number} ${anim_number_text}`;
                categoryDiv.innerHTML = `
                <div class="category-icon" style="-webkit-mask-image: url('files/${categoryData.icon}.svg'); mask-image: url('files/${categoryData.icon}.svg');"></div>
                <div id="mainContainerLeftInsideCategoryDivTexts">
                    <h4>${categoryData.label}</h4>
                    <span id="mainContainerLeftInsideCategoryDivTexts-${categoryData.name}">${categoryInfo}</span>
                </div>
                ...
                if (categoryData.number === 0 && !categoryData.isGroup) {
                    categoryDiv.style.display = "none";
                }
        ...
        if (ed.languages) {
            document.getElementById("interfaceDropdownMenu-Languages").innerHTML = "";
            ed.languages.forEach(function (langData, index) {
                var langHTML = `<a href="#" data-language="${langData.tableName}">${langData.label}</a>`;
                appendHtml(document.getElementById("interfaceDropdownMenu-Languages"), langHTML);
            });
            document.getElementById("interfaceDropdownMenu-Languages2").innerHTML = "";
            ed.languages.forEach(function (langData, index) {
                var langHTML = `<a href="#" data-language="${langData.tableName}">${langData.label}</a>`;
                appendHtml(document.getElementById("interfaceDropdownMenu-Languages2"), langHTML);
            });
        }
        if (ed.quicks) {
            for (let i = 1; i <= 7; i++) {
            ...
            quickAnimations2.forEach(function (quickData, index) {
                ...
                slotDivs.forEach(slotDiv => {
                    ...
                    quickItem.classList.add(`mainContainerRightAnimationDiv-Slot${quickData.slot}`);
                    $(quickItem).data("animData", { name: quickData.name, label: quickData.label, category: quickData.category, slot: quickData.slot, key: quickData.key });
                    slotDiv.parentNode.replaceChild(quickItem, slotDiv);
                });
                quickAnimations[quickData.slot] = {
                    imageId: quickData.imageId,
        ...
        if (ed.quicks) {
            ...
            quickAnimations2.forEach(function (quickData, index) {
                ...
                slotDivs.forEach(slotDiv => {
                    const quickItem = createQuickAnimationItem(quickData, quickData.slot);
                    quickItem.classList.add(`mainContainerRightAnimationDiv-Slot${quickData.slot}`);
                    $(quickItem).data("animData", { imageId: quickData.imageId, label: quickData.label, category: quickData.category, slot: quickData.slot, key: quickData.key });
                    slotDiv.parentNode.replaceChild(quickItem, slotDiv);
                });
                quickAnimations[quickData.slot] = {
                    imageId: quickData.imageId,
...
function getGroupMembers() {
    return Array.isArray(groupState.members) ? groupState.members : [];
}

function getGroupCategoryText() {
    if (!groupState || !groupState.enabled) return tr("group_disabled", "Disabled");
    if (!groupState.inGroup) return tr("group_no_group", "No group");
    return `${getGroupMembers().length}/${groupState.maxPlayers || 0} ${tr("group_members", "Members")}`;
}

function updateGroupCategoryText() {
    const el = document.getElementById("mainContainerLeftInsideCategoryDivTexts-group");
    if (el) el.innerHTML = getGroupCategoryText();
    document.querySelectorAll(".group-shortcut-status").forEach((status) => {
        status.innerHTML = getGroupCategoryText();
    });
    document.querySelectorAll(".group-shortcut-button").forEach((button) => {
        button.classList.toggle("group-shortcut-button-selected", groupModalOpen || currentCategory === "group");
    });
}

function getGroupActiveAnimationText() {
    if (!groupActiveAnim) return tr("group_no_selected_anim", "No animation selected yet.");
    return `${escapeHtml(groupActiveAnim.categoryLabel || groupActiveAnim.category)} / ${escapeHtml(groupActiveAnim.label || groupActiveAnim.id)}`;
}

function closeGroupModal(playClick) {
    groupModalOpen = false;
    const modal = document.querySelector(".group-modal-root");
    if (modal) modal.remove();
    if (playClick) playSound("CLICK_BACK", "WEB_NAVIGATION_SOUNDS_PHONE");
    updateGroupCategoryText();
}

function createGroupModalActionButton(label, icon, action, target, variant) {
    const button = document.createElement("button");
    button.type = "button";
    button.className = `group-modal-action-button ${variant ? "group-modal-action-button-" + variant : ""}`;
    button.innerHTML = `<i class="fa-solid ${icon}"></i><span>${label}</span>`;
    button.addEventListener("click", (e) => {
        e.stopPropagation();
        if (action === "noop") return;
        playSound("CLICK_BACK", "WEB_NAVIGATION_SOUNDS_PHONE");
        if (action === "inviteGroupPlayer") closeGroupModal(false);
...
function renderGroupModal() {
    const oldModal = document.querySelector(".group-modal-root");
    if (oldModal) oldModal.remove();
    if (!groupModalOpen) return;

    const root = document.createElement("div");
    root.className = `group-modal-root group-modal-root-${theme}`;
    const panel = document.createElement("div");
    panel.className = "group-modal-panel";
    panel.addEventListener("click", (e) => e.stopPropagation());

    const activeAnim = getGroupActiveAnimationText();
    const countText = groupState.inGroup ? `${getGroupMembers().length}/${groupState.maxPlayers || 0}` : "0";
    panel.innerHTML = `
        <div class="group-modal-header">
            <div>
                <h3>${tr("group_animation", "Group Animation")}</h3>
                <p>${tr("group", "Group")} ${countText}</p>
            </div>
            <button type="button" class="group-modal-close"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div class="group-modal-selected">
            <span>${tr("group_selected_anim", "Selected animation")}</span>
            <strong>${activeAnim}</strong>
        </div>
        <div class="group-modal-actions"></div>
        <div class="group-modal-section-title">${tr("group_members", "Members")}</div>
        <div class="group-modal-members"></div>
    `;

    panel.querySelector(".group-modal-close").addEventListener("click", () => closeGroupModal(true));

    const actions = panel.querySelector(".group-modal-actions");
    if (!groupState.enabled) {
        actions.appendChild(createGroupModalActionButton(tr("group_disabled", "Disabled"), "fa-ban", "noop", undefined, "disabled"));
    } else if (!groupState.inGroup) {
        actions.appendChild(createGroupModalActionButton(tr("group_create", "Create Group"), "fa-users", "createGroup", undefined, "primary"));
    } else if (groupState.isOwner) {
        const playAction = groupAnimationRunning
            ? { label: tr("group_stop", "Stop Group Animation"), icon: "fa-stop", action: "stopGroupAnimation", variant: "danger" }
            : { label: tr("group_start", "Start Group Animation"), icon: "fa-play", action: "startGroupAnimation", variant: "primary" };
        actions.appendChild(createGroupModalActionButton(tr("group_invite", "Invite Player"), "fa-user-plus", "inviteGroupPlayer", undefined, "primary"));
        actions.appendChild(createGroupModalActionButton(playAction.label, playAction.icon, playAction.action, undefined, playAction.variant));
        actions.appendChild(createGroupModalActionButton(tr("group_disband", "Disband Group"), "fa-xmark", "disbandGroup", undefined, "danger"));
    } else {
        actions.appendChild(createGroupModalActionButton(tr("group_leave", "Leave Group"), "fa-arrow-right-from-bracket", "leaveGroup", undefined, "danger"));
    }

    const members = panel.querySelector(".group-modal-members");
    const groupMembers = getGroupMembers();
    if (!groupState.inGroup || groupMembers.length === 0) {
        members.innerHTML = `<div class="group-modal-empty">${tr("group_no_members", "There are no group members.")}</div>`;
    } else {
        groupMembers.forEach((member) => {
            const row = document.createElement("div");
            row.className = "group-modal-member";
            const role = member.isOwner ? tr("group_owner", "Owner") : tr("group_member", "Member");
            const memberName = escapeHtml(member.name || ("ID " + member.id));
            row.innerHTML = `
                <div class="group-modal-member-avatar"><i class="fa-solid fa-user"></i></div>
                <div class="group-modal-member-info">
                    <strong>${memberName}</strong>
                    <span>${role} #${member.id}</span>
                </div>
            `;
            if (groupState.isOwner && !member.isOwner) {
                row.appendChild(createGroupModalActionButton(tr("group_kick", "Kick"), "fa-user-minus", "kickGroupMember", member.id, "mini-danger"));
            }
...
function removeGroupShortcutButtons() {
    document.querySelectorAll(".group-shortcut-button").forEach((button) => button.remove());
}

function createGroupShortcutButton(mode) {
    if (!groupCategoryData) return null;
    const button = document.createElement("button");
    button.type = "button";
    button.className = `group-shortcut-button group-shortcut-button-${mode}`;
    button.innerHTML = `
        <div class="group-shortcut-icon" style="-webkit-mask-image: url('files/${groupCategoryData.icon}.svg'); mask-image: url('files/${groupCategoryData.icon}.svg');"></div>
        <div class="group-shortcut-content">
            <span class="group-shortcut-title">${tr("group_animation", "Group Animation")}</span>
            <span class="group-shortcut-status">${getGroupCategoryText()}</span>
        </div>
    `;
    button.addEventListener("click", (e) => {
        e.stopPropagation();
...
function renderGroupShortcutButton() {
    removeGroupShortcutButtons();
    if (!groupCategoryData) return;
    const container = document.getElementById(theme === "modern" ? "mainContainer" : "mainContainer2");
    if (!container) return;
    const button = createGroupShortcutButton(theme);
    if (!button) return;
    container.appendChild(button);
    updateGroupCategoryText();
}

function createGroupActionButton(label, icon, action, target) {
    const button = document.createElement("button");
...
function renderGroupPage() {
    ...
    document.getElementById("mainContainerRightTopTitle").innerHTML = tr("group_menu", "Group Menu");
    document.getElementById("mainContainerRightTopNumber").innerHTML = groupState.inGroup ? `${getGroupMembers().length}/${groupState.maxPlayers || 0}` : "0";
    ...
    const title = groupState.inGroup ? tr("group_members", "Members") : tr("group_no_group", "You are not in a group.");
    const subtitle = groupActiveAnim
        ? `${escapeHtml(groupActiveAnim.categoryLabel || groupActiveAnim.category)} / ${escapeHtml(groupActiveAnim.label || groupActiveAnim.id)}`
        : tr("group_no_active_anim", "Play an animation first.");
    header.innerHTML = `
        <div>
            <span>${tr("group", "Group")}</span>
            <h4>${title}</h4>
        </div>
        <div class="group-page-count">${groupState.inGroup ? `${getGroupMembers().length}/${groupState.maxPlayers || 0}` : "-"}</div>
        <p>${subtitle}</p>
    `;
    ...
    if (!groupState.enabled) {
        actions.appendChild(createGroupActionButton(tr("group_disabled", "Disabled"), "fa-ban", "noop"));
    } else if (!groupState.inGroup) {
        actions.appendChild(createGroupActionButton(tr("group_create", "Create Group"), "fa-users", "createGroup"));
    } else if (groupState.isOwner) {
        const playAction = groupAnimationRunning
            ? { label: tr("group_stop", "Stop Group Animation"), icon: "fa-stop", action: "stopGroupAnimation" }
            : { label: tr("group_start", "Start Group Animation"), icon: "fa-play", action: "startGroupAnimation" };
        actions.appendChild(createGroupActionButton(tr("group_invite", "Invite Player"), "fa-user-plus", "inviteGroupPlayer"));
        actions.appendChild(createGroupActionButton(playAction.label, playAction.icon, playAction.action));
        actions.appendChild(createGroupActionButton(tr("group_disband", "Disband Group"), "fa-xmark", "disbandGroup"));
    } else {
        actions.appendChild(createGroupActionButton(tr("group_leave", "Leave Group"), "fa-arrow-right-from-bracket", "leaveGroup"));
    }
    ...
    getGroupMembers().forEach((member) => {
        ...
        const memberName = escapeHtml(member.name || ("ID " + member.id));
        item.innerHTML = `
            <div class="group-member-avatar"><i class="fa-solid fa-user"></i></div>
        ...
        `;
        if (groupState.isOwner && !member.isOwner) {
            item.appendChild(createGroupActionButton(tr("group_kick", "Kick"), "fa-user-minus", "kickGroupMember", member.id));
        }
...
function clFunc(name1, name2, name3, name4, name5, name6) {
    if (name1 === "chooseCategory") {
        ...
        playSound("CLICK_BACK", "WEB_NAVIGATION_SOUNDS_PHONE");
        post({ action: "playAnim", id: name2, category: name3, pType: name4 });
...
function createAnimationItem(item) {
    ...
    imgContainer.className = 'anim-img-container';
    ...
    imgStatic.className = 'anim-img-static';
    imgStatic.src = `${PREVIEW_STATIC_BASE}/${imgName}.webp?v=9999`;
    imgStatic.style.position = 'absolute';
    imgStatic.style.width = '23.111vh';
    imgStatic.style.top = '-2.5vh';
    imgStatic.style.left = '50%';
    imgStatic.style.transform = 'translateX(-50%)';
    imgStatic.style.opacity = '1';
    imgStatic.style.transition = 'opacity 0.15s ease';
    imgStatic.onerror = () => {
        imgStatic.src = "files/unknown.png";
        imgStatic.style.width = "14.222vh";
        imgStatic.style.top = "0";
    };
    ...
    imgHover.className = 'anim-img-hover';
    imgHover.style.position = 'absolute';
    imgHover.style.width = '23.111vh';
    imgHover.style.top = '-2.5vh';
    imgHover.style.left = '50%';
    imgHover.style.transform = 'translateX(-50%)';
    imgHover.style.opacity = '0';
    imgHover.style.transition = 'opacity 0.15s ease';
    if (item.category === "expressions") {
        imgHover.style.width = "14.222vh";
        imgHover.style.top = "0";
    }
    imgContainer.appendChild(imgStatic);
    imgContainer.appendChild(imgHover);
    div.appendChild(imgContainer);
    let hoverImageLoaded = false;
    const hoverImageUrl = `${PREVIEW_HOVER_BASE}/${imgName}.webp?v=9999`;
    div.addEventListener("mouseenter", () => {
        if (!hoverImageLoaded) {
            imgHover.src = hoverImageUrl;
            hoverImageLoaded = true;
        }
        imgStatic.style.opacity = '0';
...
function showNotify(title, message) {
    post({ action: "notify", title: title, message: message });
}
...
function addAnimToQuick(data, id) {
    ...
    slotDivs.forEach(slotDiv => {
        ...
        slotDiv.addEventListener('click', (e) => {
            e.stopPropagation();
            clFunc('playAnim', data.imageId, data.category, "quicks");
        });
...
function createQuickAnimationItem(data, slotId) {
    ...
    img.src = `${PREVIEW_STATIC_BASE}/${data.imageId}.webp?v=9999`;
    img.onerror = () => {
    ...
    img.addEventListener("mouseenter", () => {
        img.src = `${PREVIEW_HOVER_BASE}/${data.imageId}.webp?v=9999`;
    });
    ...
    commandSpan.textContent = translations.key + " " + key;
    commandDiv.appendChild(commandSpan);
    div.appendChild(commandDiv);
    div.addEventListener('click', (e) => {
        e.stopPropagation();
...
//                 localStorage.setItem('themev2', 'modern'); 
//             } else {
//                 localStorage.setItem('themev2', 'compact'); 
//             }
//         }
//     });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    toggleButtons.forEach((toggleButton, index) => {
        ...
        dropdownMenu.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'A') {
                if (event.target.dataset.interface) {
                    const selectedInterface = parseInt(event.target.dataset.interface);
                    interface1Content.style.display = 'none';
                    interface2Content.style.display = 'none';
                    currentInterface = selectedInterface;
                ...
                if (event.target.dataset.language) {
                    ...
                    interface2Content.style.display = 'none';
                    dropdownMenu.style.display = 'none';
                    post({ action: "setLanguage", language: event.target.dataset.language });
                }
                menuOpen = false;
                closeGroupModal(false);
                post({ action: "close" });
