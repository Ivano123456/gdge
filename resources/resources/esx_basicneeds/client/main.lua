...
AddEventHandler('esx_status:loaded', function()
    local function registerStatus(name, color, removalRate)
        TriggerEvent('esx_status:registerStatus', name, 1000000, color,
            ...
            function(status)
                if GetResourceState('jamaica-safezone') == 'started' and exports['jamaica-safezone']:ShouldFreezeNeeds() then
                    return
                end
...
AddEventHandler('esx_status:onTick', function(statuses)
    ...
    local freezeNeeds = GetResourceState('jamaica-safezone') == 'started' and exports['jamaica-safezone']:ShouldFreezeNeeds()

    for _, status in pairs(statuses) do
        if status.percent == 0 and not (freezeNeeds and (status.name == 'hunger' or status.name == 'thirst')) then
