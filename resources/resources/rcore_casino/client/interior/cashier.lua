...
function Cashier_Load()
    ...
    for _, o in pairs(CashierDatas) do
        if o.enabled then
            ...
            if Config.MapType == 16 then
                -- sit animation
                SetEntityCollision(cashier.ped, false, true)
                FreezeEntityPosition(cashier.ped, true)
                RequestAnimDictAndWait("anim@amb@clubhouse@seating@female@var_a@base@")
                TaskPlayAnim(cashier.ped, "anim@amb@clubhouse@seating@female@var_a@base@", "base", 8.0, -8.0, -1, 1, 0, false, false, false)
            end
