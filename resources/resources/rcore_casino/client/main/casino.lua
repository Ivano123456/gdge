...
local function PrepareMapOnEnter()
    ...
    elseif Config.MapType == 14 then
        ...
        CreateSwap(-1916.89, -1337.52, -3.659, 2.0, GetHashKey("vw_prop_casino_blckjack_01b"),
            GetHashKey("vw_prop_casino_3cardpoker_01"))
    elseif Config.MapType == 16 then
        local extraWheel = GetClosestObjectOfType(295.20, 275.94, 89.74, 2.0, -1901044377)
        if extraWheel ~= 0 then
            ForceDeleteEntity(extraWheel)
        end
        local extraCircle = GetClosestObjectOfType(295.20, 275.94, 89.74, 2.0, -945995088)
        if extraCircle ~= 0 then
            ForceDeleteEntity(extraCircle)
        end
    end
end
...
function OnEnterCasino()
    ...
    -- Prompt: Chinese Casino has custom slot model
    machineModels[207578973].model = Config.MapType == 16 and "i45pt_ch_casino_slotmachine_01a" or "vw_prop_casino_slot_07a"
