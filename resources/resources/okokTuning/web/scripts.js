...
let resprayOptions = {
  ...
  filterOptions: () => {
    if (hideMods) {
      resprayOptions.full = resprayOptions.full.filter((option) => {
        let shouldInclude = true;
        for (let cat in hideMods) {
          if (option.id === cat) {
            shouldInclude = false;
            break;
          }
      ...
      resprayOptions.motorcycle = resprayOptions.motorcycle.filter((option) => {
        let shouldInclude = true;
        for (let cat in hideMods) {
          if (option.id === cat) {
            shouldInclude = false;
...
function HandleCategory(element) {
  ...
  if (neons.list.includes(id)) {
    ...
    if (id === resprayOptions.list[0] || id === resprayOptions.list[1]) {
      ...
      } else if (hideMods["chameleon"]) {
        CreatePrimaryRGBColorMenu(Locale("PrimaryColor"));
      } else if (hideMods["rgb"]) {
        CreateChameleonColorMenu(Locale("PrimaryColor"));
      } else {
