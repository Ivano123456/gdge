...
AddEventHandler(Config.EventPrefix..":OpenSelectionMenu", function()
	...
	for k,v in pairs(Config.AllowedSocieties) do
		for k2, v2 in pairs(Config.SocietyAccessRanks) do
			if PlayerJobName == v and PlayerJob.grade_name == v2 or Config.SocietyAccessRanks[1] == '' then 
		...
		for k,v in pairs(Config.JobsWithCityInvoices) do
			if PlayerJobName == v then 
				for k2, v2 in pairs(Config.CityInvoicesAccessRanks) do
					if PlayerJob.grade_name == v2 or Config.CityInvoicesAccessRanks[1] == '' then 
	...
	ESX.TriggerServerCallback(Config.EventPrefix..":getPlayerGroup", function(playerGroup)
		ESX.TriggerServerCallback(Config.EventPrefix..":getPlayerInvoices", function(pInvoices)
			isInvoicesOpen = true
