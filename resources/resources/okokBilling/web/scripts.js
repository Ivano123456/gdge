// Security: escapes all HTML from user/server supplied strings to prevent XSS
	...
	switch (event.data.action) {
		...
		case 'reference':
			...
			if(invoice[0] != undefined && ["ref_id"] != undefined && invoice[0]["status"] == "unpaid"){
				$('#payreference_receiver').html(safeText(invoice[0]["receiver_name"]))
				$('#payreference_amount').html(safeText(invoice[0]["invoice_value"]))
	...
	let newData = {
		data: [
			[
				...
				`<div class="text-center align-middle">${safeText(invoice["author_name"])}</div>`,
				`<div class="text-center align-middle">${safeText(invoice["invoice_value"])}€</div>`,
...
function openClosePlayers(){
	if(nearPlayers.length > 0){
		...
		for(var i = 0; i < nearPlayers.length; i+=1){
			...
			if(addedCards == 2){
				var safeId2 = parseInt(nearPlayers[i]["id"]) || 0;
				var safeName2 = safeText(nearPlayers[i]["name"]);
				element += `
				<div class="col-md-6 pl05">
					<button type="button" onclick="createInvoice(${safeId2})" class="btn btn-odark2 fs125 h-100 w-100" id="${safeId2}" data-bs-dismiss="modal">${safeName2} (${safeId2})</button>
				</div>`;
				element += `</div>`;
				addedCards = 0;
			} else {
				var safeId = parseInt(nearPlayers[i]["id"]) || 0;
				var safeName = safeText(nearPlayers[i]["name"]);
				element += `
				<div class="col-md-6 pr05">
					<button type="button" onclick="createInvoice(${safeId})" class="btn btn-odark2 fs125 h-100 w-100" id="${safeId}" data-bs-dismiss="modal">${safeName} (${safeId})</button>
				</div>`;
			}
		}
		element += `</div>`;
...
function renderDetails(id) {
	...
	$(`#${invoice['status']}_view_invoice_number`).html(safeText(invoice["id"]))
	$(`#${invoice['status']}_view_invoice_id`).html(safeText(invoice["ref_id"]))
	$(`#${invoice['status']}_view_invoice_sent`).html(safeText(invoice["sent_date"]))
	$(`#${invoice['status']}_view_invoice_due`).html(safeText(invoice["limit_pay_date"]))
	$(`#${invoice['status']}_view_invoice_from`).html(safeText(invoice["author_name"]))
	$(`#${invoice['status']}_view_invoice_to`).html(safeText(invoice["receiver_name"]))
	if(invoice['society'] == ""){
		$(`#${invoice['status']}_view_invoice_from`).html(safeText(invoice["author_name"]))
	}
	else{
		$(`#${invoice['status']}_view_invoice_from`).html(safeText(invoice["society_name"]))
	}
	$(`#${invoice['status']}_view_invoice_item`).html(safeText(invoice["item"]))
	...
	$(`#${invoice['status']}_view_invoice_note`).html(safeText(invoice["notes"]))
	$(`#${invoice['status']}_view_invoice_pay`).attr('onclick', `payInvoice("${invoice["ref_id"]}", "${type}")`);

	if(invoice['status'] == 'paid'){
		$('#paid_view_invoice_paid_date').html(safeText(invoice['paid_date']))
	}
