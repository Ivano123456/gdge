	...
	while true do
		...
		if not Config.GUI.Toggle then
			if IsControlPressed(1, Config.GUI.Key) then
				...
				if Check(Ped) then MenuOpened = true sleep = 0 end
			else MenuOpened = false end
			if IsControlJustPressed(1, Config.GUI.Key) then
				local Ped = PlayerPedId() 
				if Check(Ped) then SoundPlay("Open") SetCursorLocation(Config.GUI.Position.x, Config.GUI.Position.y) sleep = 0 end
			elseif IsControlJustReleased(1, Config.GUI.Key) then
				if Check(Ped) then MenuOpened = false SoundPlay("Close") sleep = 0 end
			end
		else
			if IsControlJustPressed(1, Config.GUI.Key) then
				local Ped = PlayerPedId() 
				if Check(Ped) then SoundPlay("Open") SetCursorLocation(Config.GUI.Position.x, Config.GUI.Position.y) MenuOpened = not MenuOpened sleep = 0 end
			end
		end
		if MenuOpened then 
			DrawGUI() 
